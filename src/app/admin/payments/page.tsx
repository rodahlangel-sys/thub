              ...
              <table className="min-w-[1120px] w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-[#edf1f3]">
                  {payments.map((payment) => (
                    <tr className="text-[#244b5b]" key={payment.id}>
                      <td className="px-4 py-4 font-medium text-[#182f38]">{formatShortOrderId(payment.id)}</td>
                      <td className="px-4 py-4">
                        <Link className="font-semibold text-[#176b87] hover:text-[#12566d]" href={`/admin/orders/${payment.orderId}`}>
                      ...
                      <td className="px-4 py-4">{getPaymentProviderLabel(payment.provider)}</td>
                      <td className="px-4 py-4">
                        <Badge tone={getPaymentStatusTone(payment.status)}>
                          {getPaymentStatusLabel(payment.status)}
                        </Badge>
                      </td>
                      <td className="px-4 py-4">{formatOrderMoney(payment.amount)}</td>
                      <td className="max-w-64 break-all px-4 py-4">{payment.transactionNo || "暂无"}</td>
                      <td className="px-4 py-4">{payment.paidAt ? formatDateTime(payment.paidAt) : "暂无"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
